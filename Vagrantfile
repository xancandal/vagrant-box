...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  # Forward the output of graphical programs using X11 to our host machine
  ...
  #   # Use VBoxManage to customize the VM. For example to change memory:
    ...
    # (at least .5 Gig per CPU. 2.5 Gig Minimum to run the testHarness)
    vb.memory=512*[(1+nrCPU),5].max
    # Customize video car VRAM
    vb.customize ["modifyvm", :id, "--vram", "256"]
    # host CPU execution cap of 50%
